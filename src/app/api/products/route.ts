...
export async function GET(request: Request) {
  ...
  try {
    ...
    const isActive = String(searchParams.get('is_active') || '').trim();
    ...
    return NextResponse.json({
      ...
      page_info: payload.page_info || {
        ...
        has_more: false,
        next_cursor: null,
        total_returned: Array.isArray(payload.products) ? payload.products.length : 0,
      },
    });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await backendAdminFetchJson<{ product?: { id?: number | string } }>(
      '/api/admin/products',
      {
        method: 'POST',
